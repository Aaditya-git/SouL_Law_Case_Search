...
# # Define mock data for cases in various countries
# mock_cases = {
#     "USA": [
...
# # Index each case into Elasticsearch
# def index_case(country, case_data):
#     for case in case_data:
#         # Randomly generate an ID for each case
#         case_id = fake.uuid4()
#         # Index case data into Elasticsearch (use the fixed index name 'case_laws')
#         es.index(index="case_laws", id=case_id, document=case)

# # Index the cases
# for country, cases in mock_cases.items():
#     index_case(country, cases)
...
es = Elasticsearch(
    ELASTIC_URL,
    basic_auth=(ELASTIC_USER, ELASTIC_PASSWORD),
    verify_certs=False  # Disable cert verification for self-signed certs, change to True for production
)
...
# Define the mapping for the case_laws index (Optional but recommended)
index_mapping = {
    "mappings": {
        "properties": {
...
# Function to prepare case data for bulk indexing
def prepare_bulk_data(country, case_data):
    actions = []
    for case in case_data:
        case_id = fake.uuid4()  # Randomly generate an ID for each case
        action = {
            "_op_type": "index",
            "_index": "case_laws",
            "_id": case_id,
...
def index_cases_in_bulk():
    ...
    # Use the helpers.bulk function to index in bulk
